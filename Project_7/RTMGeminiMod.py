...
def toggle_pause():
    if player.active:
        if player.paused:
            ...
            # Prompt user
            ans = messagebox.askyesno("Paused", "Playback Paused.\n\nClick YES to upload a new PDF, or NO to continue playing.")
            if ans:
                # User wants a new PDF
                player.stop()
...
def stop_audio():
    if player.active:
        ...
        ans = messagebox.askyesno("Stopped", "Playback Stopped.\n\nClick YES to upload a new PDF, or NO to just remain stopped.")
...
def process_content():
    ...
    try:
        ...
        # -------------------------------------------------------------
        # FIXED HIGHLIGHT MAPPING
        # Instead of manual math, we rely on Tkinter's native char counts
        # -------------------------------------------------------------
        ui_word_timings = []
        char_offset = 0
        ...
        for wb in word_boundaries:
            ...
            if idx != -1:
                # Ask Tkinter to translate the absolute character position into a line.col index
...
# Updated highlight configuration to high-contrast Yellow & Black
